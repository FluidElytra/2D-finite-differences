...
function Graph:new(position, dimension, x_scale, y_scale)
	...
	-- self.origin = Vector(self.position.x + self.dimension.x/2, self.position.y + self.dimension.y/2)
	self.origin = Vector(self.position.x, self.position.y+ self.dimension.y)
	self.xlabel = ''
	self.ylabel = ''
	self.cross = false
end
...
function Graph:draw()
	...
	for i = 1,tic_n do
		...
		-- tick label
		love.graphics.printf(tostring(label),CMU_serif,tic_x-15,tic_y+5,30,"center",0,1,1)

		tic_x = tic_x + tic_step
		label = label + tic_step*self.x_unit2pixels
	end

	love.graphics.printf(self.xlabel,CMU_serif_italic,self.position.x+0.5*self.dimension.x-40,self.position.y+self.dimension.y+20,200,"center",0,1,1)
	...
	for i = 1,tic_n do
		...
		-- tick label
		love.graphics.printf(tostring(label),CMU_serif,self.position.x-30,tic_y-10,30,"center",0,1,1)
		tic_y = tic_y - tic_step
		label = label + tic_step*self.y_unit2pixels
	end

	love.graphics.printf(self.ylabel,CMU_serif_italic,self.position.x-120,self.position.y+0.5*self.dimension.y-20,90,"center",0,1,1)
...
function Graph:plot(points, color)
	...
	if #points > 0 then
		for i = 1,#points do
			...
			local x_graph = self.origin.x + points[i].x / self.x_unit2pixels
			local y_graph = self.origin.y - points[i].y / self.y_unit2pixels
			-- plot the points
